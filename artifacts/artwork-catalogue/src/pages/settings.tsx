...
const BUILT_IN_CURRENCIES = [
  "USD", "EUR", "GBP", "CHF", "JPY", "CNY", "AUD", "CAD", "HKD", "NGN",
];
...
export default function Settings() {
  ...
  async function handleSave() {
    ...
    // If usePassword is enabled but no password has ever been set, warn
    if (usePassword && !passwordHash) {
      setPasswordMsg("Please set a password before enabling login protection.");
      return;
    }

    saveSettings({
      ...settings,
      collectionOwner: collectionOwner.trim(),
      userEmail: userEmail.trim(),
      passwordHash,
      usePassword: usePassword && !!passwordHash,
      openingMode,
      pinnedArtworkId,
    });
  ...
  function addCurrency() {
    ...
    if (!/^[A-Z]{2,6}$/.test(code)) {
      setCurrencyError("Currency codes must be 2–6 letters (e.g. NGN, EUR).");
      return;
    }
    if (BUILT_IN_CURRENCIES.includes(code)) {
      setCurrencyError(`${code} is already included by default.`);
      return;
    }
    if (settings.customCurrencies.includes(code)) {
      setCurrencyError(`${code} is already in your list.`);
      return;
    }
    setCurrencyError("");
    setNewCurrency("");
  ...
  function removeCurrency(code: string) {
    saveSettings({
      ...settings,
      customCurrencies: settings.customCurrencies.filter((c) => c !== code),
    });
  }
    ...
    <div className="max-w-lg space-y-12">
      <header>
        ...
        <p className="text-muted-foreground mt-2 font-light">
          Preferences for your collection registry.
        </p>
      </header>
      ...
      <section className="space-y-4 border-t border-border pt-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mb-5">
            The owner's name appears at the top of the sidebar.
          </p>
        </div>
      ...
      <section className="space-y-5 border-t border-border pt-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Your email is stored locally as an identifier for future password recovery.
          </p>
        </div>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="new-password" className="text-sm text-muted-foreground">
              {settings.passwordHash ? "New password" : "Password"}
            </Label>
          ...
          {passwordMsg && (
            <p className={`text-xs ${passwordMsg.includes("updated") ? "text-green-600" : "text-destructive"}`}>
              {passwordMsg}
            </p>
          )}
        </div>
        ...
        <div className="flex items-start gap-3 pt-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              When enabled, a password prompt appears each time the app is opened in a new browser session.
              {!settings.passwordHash && " You must set a password above first."}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-5 border-t border-border pt-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Choose what appears on the home screen when you open the app.
          </p>
        </div>

        <RadioGroup
          value={openingMode}
          onValueChange={(v) => setOpeningMode(v as "fixed" | "random")}
          className="space-y-4"
        >
          <div className="flex items-start gap-3">
            ...
            <Label htmlFor="mode-random" className="cursor-pointer space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground font-normal">
                A different work from the collection is shown each time.
              </p>
            </Label>
          </div>
          <div className="flex items-start gap-3">
            ...
            <Label htmlFor="mode-fixed" className="cursor-pointer space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground font-normal">
                Always show the same work — you choose which one.
              </p>
            </Label>
          </div>
        </RadioGroup>
        ...
        {openingMode === "fixed" && (
          <div className="pl-6 space-y-2">
            ...
            <Select
              value={pinnedArtworkId ? String(pinnedArtworkId) : ""}
              onValueChange={(v) => setPinnedArtworkId(v ? Number(v) : null)}
            >
              ...
              <SelectContent>
                {artworks.map((a) => (
                  <SelectItem key={a.id} value={String(a.id)}>
                    ...
                    {a.artist && (
                      <span className="text-muted-foreground ml-2 text-xs">— {a.artist}</span>
                    )}
                  </SelectItem>
                ))}
              </SelectContent>
      ...
      <div className="flex items-center gap-4 pt-2">
        ...
        {saved && (
          <span className="text-xs text-muted-foreground animate-in fade-in duration-200">Saved.</span>
        )}
      </div>
      ...
      <section className="space-y-5 border-t border-border pt-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Add extra currency codes to use in pricing. Standard currencies are already included.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {BUILT_IN_CURRENCIES.map((code) => (
            <span
              key={code}
              className="inline-flex items-center px-2.5 py-1 rounded-sm text-xs font-mono bg-muted text-muted-foreground border border-border"
            >
              {code}
            </span>
          ))}
          {settings.customCurrencies.map((code) => (
            <span
              key={code}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-sm text-xs font-mono bg-background border border-border"
            >
              {code}
              <button
                type="button"
        ...
        <div className="flex gap-2 items-start">
          <div className="space-y-1">
            <Input
              ...
              value={newCurrency}
              onChange={(e) => { setNewCurrency(e.target.value.toUpperCase()); setCurrencyError(""); }}
