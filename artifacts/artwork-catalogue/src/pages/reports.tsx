...
function formatDimensions(a: Artwork) {
  ...
  return parts.join(" × ") + ` ${a.dimensionUnit || "cm"}`;
}
...
export default function Reports() {
  ...
  const allArtists = useMemo(
    () => [...new Set((artworks || []).map((a) => a.artist).filter(Boolean))] as string[],
    [artworks]
  );
  const allMediums = useMemo(
    () => [...new Set((artworks || []).map((a) => a.medium).filter(Boolean))] as string[],
    [artworks]
  );
  const allLocationNames = useMemo(
    () => [...new Set((artworks || []).map((a) => a.locationName).filter(Boolean))] as string[],
    [artworks]
  );
  ...
  const filteredArtworks = useMemo((): Artwork[] => {
    ...
    switch (reportType) {
      case "on_loan":
        return artworks.filter((a) => a.onLoan) as Artwork[];

      case "by_artist":
        return (filterValue
          ? artworks.filter((a) => a.artist === filterValue)
          : artworks) as Artwork[];

      case "by_location":
        return (filterValue
          ? artworks.filter((a) => a.locationName === filterValue)
          : artworks) as Artwork[];

      case "by_medium":
        return (filterValue
          ? artworks.filter((a) => a.medium === filterValue)
          : artworks) as Artwork[];

      case "acquired_last_30":
        return artworks.filter(
          (a) => new Date(a.createdAt).getTime() > daysAgo(30)
        ) as Artwork[];

      case "acquired_last_60":
        return artworks.filter(
          (a) => new Date(a.createdAt).getTime() > daysAgo(60)
        ) as Artwork[];

      case "acquired_last_90":
        return artworks.filter(
          (a) => new Date(a.createdAt).getTime() > daysAgo(90)
        ) as Artwork[];
  ...
  const getReportTitle = () => {
    switch (reportType) {
      case "by_artist":
        return filterValue ? `Works by ${filterValue}` : "All Artworks — By Artist";
      case "by_location":
        return filterValue ? `Works at ${filterValue}` : "All Artworks — By Location";
      case "by_medium":
        return filterValue ? `Works in ${filterValue}` : "All Artworks — By Medium";
      case "on_loan":
        return "Works Currently On Loan";
      case "acquired_last_30":
        return "Acquired in the Last 30 Days";
      case "acquired_last_60":
        return "Acquired in the Last 60 Days";
      case "acquired_last_90":
        return "Acquired in the Last 90 Days";
      case "acquired_custom": {
        const parts = [];
      ...
      <div className="print:hidden space-y-8">
        <header>
          ...
          <p className="text-muted-foreground mt-1 font-light">
            Generate and print collection reports.
          </p>
        </header>
        ...
        <div className="border border-border p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Dropdown filter for by_artist / by_location / by_medium */}
            {showDropdownFilter && (
              <div>
                <label className="text-xs text-muted-foreground mb-1.5 block uppercase tracking-widest">
                ...
                <Select
                  ...
                  onValueChange={(v) => setFilterValue(v === "_all" ? "" : v)}
                >
                  <SelectTrigger>
            ...
            {/* Artist filter for acquired_artist_date */}
            {showArtistFilter && (
              <div>
                <label className="text-xs text-muted-foreground mb-1.5 block uppercase tracking-widest">
                  Artist
                ...
                <Select
                  value={artistFilter || "_all"}
                  onValueChange={(v) => setArtistFilter(v === "_all" ? "" : v)}
                >
                  <SelectTrigger>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-border">
            <p className="text-sm text-muted-foreground">
              {isLoading
                ? "Loading..."
                : `${filteredArtworks.length} artwork${filteredArtworks.length !== 1 ? "s" : ""} in report`}
            </p>
            <Button
        ...
        <div className="print-report">
          ...
          <div className="mb-10 print:mb-8">
            <div className="border-b-2 border-foreground pb-4 mb-6">
              <p className="text-[10px] tracking-widest uppercase text-muted-foreground print:text-black/50 mb-2">
                Hourglass · Private Registry
              </p>
          ...
          {filteredArtworks.length === 0 ? (
            <div className="text-center py-12 print:hidden">
              ...
              <p className="text-muted-foreground text-sm">
                No artworks match the selected criteria
              </p>
            </div>
          ) : (
            <div className="space-y-6 print:space-y-4">
              {filteredArtworks.map((artwork, i) => (
                <div
                  key={artwork.id}
                  className="flex gap-6 py-5 border-b border-border last:border-0 print:py-4 print:break-inside-avoid"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground print:text-black/60 mt-0.5">
                    ...
                    <div className="mt-2 flex flex-wrap gap-x-6 gap-y-0.5">
                      ...
                      {artwork.onLoan && (
                        <span className="text-xs text-muted-foreground print:text-black/60 italic">
                          On loan
                        </span>
                      )}
                      <span className="text-xs text-muted-foreground print:text-black/60">
